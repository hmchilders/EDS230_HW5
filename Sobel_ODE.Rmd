...
4.  Now lets run a sensitivity analysis for our parameters
    ...
    #------------------------------------------------
    #--------Run the ODE Solver with Sobel set--------
    #-------------------------------------------------
    # Create a function to cmopute the maximum forest size
...
5.  Graph the results and record the S & T Sobel indeces
...
From what I am seeing based on my Sobel indices, the linear growth rate that occurs after the threshold is reached is not as important in determining the max forest size as the exponential growth rate and the carrying capacity. This is important as the climate continues to change because I would anticipate that climate change will cause the carrying capacity to decrease. As we get hotter summers and wetter winters, the number of trees a forest can support will be limited to how many can survive through the hot summer. This logic also allpies to the exponential growth rate. As we get hotter summers and wetter winters, the exponential growth rate will vary drastically based on the season. This will seriously impact maximum forest size as the exponential growth rate is the most influential parameter that we tested.
